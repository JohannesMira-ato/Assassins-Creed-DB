...
def login():
    ...
    if request.method == "POST":
        ...
        user = db.fetch("""SELECT Username, Password FROM UserInfo WHERE
...
def database_add():
    if 'username' in session:
        return render_template('database_add.html', username=session['username'])
    else:
...
def database_character_add():
    ...
    if 'username' in session:
        ...
        if request.method == "POST":
            ...
            db.character(name=name, alias=alias, birthdate=birthdate,
